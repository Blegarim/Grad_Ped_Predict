...
| 7.1 | `export/onnx.py`, `scripts/export_onnx.py`, `config/schema.py` (`ExportCfg`), `configs/export.yaml` | `onnx/onnx_export.py` | n/a (export infrastructure; no numeric golden) | B2 | ✅ | All 4 model types exported via typed registry. Dynamic axes on batch (0) AND seq_len (1) — upgrade over legacy batch-only. `check_onnx_parity` is a first-class function (onnxruntime `CPUExecutionProvider`; tolerances from `ExportCfg`). `ExportCfg` adds `opset`/`output_dir`/`include_temporal_weights`/`parity_atol`/`parity_rtol`/`parity_batch_size`/`parity_seq_len`; flows through `loader.py` + `configs/export.yaml`. `crosses_pooled` excluded from ONNX output (B4 — unsupervised). `temporal_weights` opt-in (`include_temporal_weights=False` default → 3-key legacy contract). **B2 verified**: `test_export_without_prior_forward` confirms `torch.onnx.export` traces cleanly with no prior `model.forward()` call (all ViT params exist at `__init__`). 8 tests: B2 guard, validity×4 types, output names×4, parity random×4, parity golden weights, dynamic batch, dynamic seq_len. |
| 8.1 | `tests/_golden.py`, `tests/test_golden_outputs.py`, CI gate | OLD `test_*.py` ad-hoc scripts | registry over all 14 `fixtures/golden/*` | B12 | ✅ | Suite already built incrementally (24 module test files + 12 capture scripts + golden fixtures + `ci.yml`). 8.1 closes B12 *structurally*: `_golden.GOLDEN_MANIFEST` registers every fixture→capture-script→guarded-module; `test_golden_outputs.py` meta-tests enforce present/no-orphan/regenerator-exists/referenced-by-a-test, so porting a module without a golden guard now FAILS. Resume (G3) already covered (`test_callbacks::test_save_resume_continue_equivalence` + `test_build_trainer_with_resume`, `test_trainer::test_state_dict_round_trips_strict_without_forward`/B2); ONNX parity (G1) already covered (`test_onnx_export.py`, module-level `importorskip` keeps gate green w/o `[export]`). OLD ad-hoc scripts folded/dropped (see disposition table). CI: `ruff` + `pytest -m "not slow"` + soft `--cov` term-missing (no hard floor yet). 43 new meta-test cases green (41 passed + 2 skipped offline-capture); full gate 390 passed / 3 skipped. |
| 8.2 | `CLAUDE.md`, `README.md` | OLD `CLAUDE.md`, `README.md`, `GUIDELINE.md` | n/a | B6 (docs), B12 (docs) | ✅ | **Hybrid doc-shift (decided this prompt): segregate now, flip at 9.1.** CLAUDE.md restructured to read as standalone project docs with rebuild scaffolding quarantined into one trailing **"Rebuild Context (Phase A — retire at cutover)"** section (legacy ref + two-phase plan + band-aid table) — load-bearing until P9, then deleted along with archived SCHEMATIC/MIGRATION. Drift fixed: stale "not yet implemented" status → "Phase A complete P0–P8, P9 pending"; layout reconciled to disk (+`geometry`,`schedule`,`lmdb_warm`,`infer_video`, all `configs/*.yaml`); **CLI override syntax corrected `--train.lr 5e-5` → `--set train.lr=5e-5`** (matched code/README); new **Commands** section; prose "keep in sync" bullet → structured **Doc-Sync Checklist**. README layout aligned (+`geometry`/`lmdb_warm`/`schedule`); already standalone otherwise. Module docstrings audited — all 38 present + provenance-tagged, no edits. All doc-referenced paths verified to exist on disk. Module docstring de-tagging (prompt-number provenance) + full standalone flip deferred to 9.1. |
