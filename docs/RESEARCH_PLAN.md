...
**Phase B has begun.** A full two-pass code audit ([`HOLE_AUDIT.md`](HOLE_AUDIT.md)) catalogued and
resolved every validity-, design-, and correctness-level issue in v1.0. Its first batch — the
data-independent code fixes (reproducible seeding, F1-based model selection, val-tuned decision
thresholds, an imbalance-distribution instrument, and a set of latent-bug fixes) — is already
implemented and committed. This engineering foundation is what makes the research phase feasible:
every proposed change can be ablated against a trusted, reproducible, leakage-free baseline.
...
1. **Data-independent code fixes — DONE (committed).** Reproducible seeding (`train.seed`), F1-based
...
2. **The ONE v2 dataset rebuild** (sequences + LMDBs, all three splits; the ~20k v1 train chunks are
   discarded, the v1 build is *not* resumed). Batched so the dataset is touched exactly once:
   - relabel `actions`/`looks` as state-at-end-of-observation (`crosses` stays future-any); measure
     and re-pin the class-rate shift;
   - drop right-censored windows (unobserved futures silently labeled 0); measure the excluded count;
   - carry a `track_id` into the LMDB metadata (enables track-aggregated metrics);
   - motion-v2 data fixes (frame-0 deltas = 0; flip reflects absolute cx) and an **ego-speed** channel
     (stored wide, sliced to `motion_dim` at load — with/without ego is two configs, no rebuild);
   - a benchmark-protocol evaluation set (test-split only, same writer pass) for literature comparison.
   - Doc-sync: update the Dataset Statistics table, `count_labels` gate, `test_stats` fixture, goldens.
3. **The hub ladder (~6 runs).** Registry/rename wave first (add a pixel-free **kinematics-only**
